...
bool Win_QextSerialPort::open(OpenMode mode) {
    ...
    if (!isOpen()) {
        /*open the port*/
        QString strPortName = QString("\\\\.\\%1").arg(port);
        Win_Handle=CreateFileA(strPortName.toStdString().c_str(), GENERIC_READ|GENERIC_WRITE,
...
void Win_QextSerialPort::setTimeout(ulong sec, ulong millisec) {
    ...
    if(isOpen()) {
        Win_CommTimeouts.ReadIntervalTimeout = MAXDWORD/*sec*1000+millisec*/;
        Win_CommTimeouts.ReadTotalTimeoutMultiplier = 0/*sec*1000+millisec*/;
